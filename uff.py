...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def get_topology_data(self,ev):
        ...
        self.host_list=copy.copy(get_host(self,None))
        ...
        for host in self.host_list:
            ...
            mac=host.mac
            ip=host.ipv6[0]
            self.host_info[(switch,port)]={"mac":mac,"ip":ip}
    ...
    def _monitor_port(self, dpid, port_no):
        ...
        for(dpid, port_no) in self.host_info:
            if (dpid, port_no) in active_sp and self.host_info[(dpid,port_no)].get('mac') in ?? :
        ...
        if rx_throughput > self.threshold:
            if ((dpid, port_no) not in self.monitoring_list and (dpid, port_no) not in self.blocked_ports):
                ...
                if (dpid,port_no) in self.host_info_updated:
            ...
            elif (dpid, port_no) in self.monitoring_list and (dpid, port_no) not in self.blocked_ports and (dpid,port_no) in self.host_info_updated:
                self.logger.warning(f'\n*************LA PORTA {(dpid, port_no)} HA SUPERATO LA SOGLIA CON RX=%f*************', rx_throughput)
