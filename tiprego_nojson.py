...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def get_host_info(self):
        self.logger.info("Fetching host information from the SDN controller...")
    
        # Ottenere gli host attivi
        hosts = get_host(self, None)
        
        # Popolare self.host_info con le informazioni sugli host
        for host in hosts:
            # Creare una chiave unica per l'host usando DPID e numero della porta
            key = (host.port.dpid, host.port.no)
            # Memorizzare MAC e IP dell'host
            self.host_info[key] = {
                "mac": host.mac,
                "ip": host.ipv4[0] if host.ipv4 else None  # Assicurati di prendere il primo indirizzo IP
            }

    self.logger.info(f"Host information: {self.host_info}")
    print("***HOST****\n")
