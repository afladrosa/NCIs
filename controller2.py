...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _block_port(self, dpid, port_no):
        del self.monitoring_list[(dpid, port_no)]
        self.logger.info(f'\nRIMUOVO monitoring_list: {self.monitoring_list}\n')
        ...
        for stat in body:
            ...
            if port_no not in self.port_stats[dpid]:
                ...
                if dpid == 3:
                    ...
                    if rx_throughput > dynamic_threshold: #rimosso controllo porte attive
                        ...
                        elif (dpid, port_no) in self.monitoring_list not in self.blocked_ports:
                            sself.blocked_ports[(dpid, port_no)] = 0
                            self._block_port(dpid, port_no)

                        self.logger.info(f'\n AGGIUNGO monitoring_list: {self.monitoring_list}\n')
                    elif rx_throughput < dynamic_threshold:
