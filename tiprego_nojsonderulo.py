...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def get_topology(self):
        # Costruisce la topologia basata sugli host
        self.logger.info("Current topology: %s", self.host_info)
    ...
    def _monitor_port(self, dpid, port_no):
        ...
        if rx_throughput > self.threshold:
            ...
            if (dpid, port_no) in self.monitoring_list:
                self.monitoring_list.remove((dpid, port_no))
                self.logger.info(f'\n*************LA PORTA {(dpid, port_no)} È STATA RIMOSSA DALLA monitoring_list: {self.monitoring_list}*************')
    ...
    def _flow_stats_reply_handler(self, ev):
        ...
        for stat in body:
            if stat.match['eth_type'] == ether_types.ETH_TYPE_ARP:
                # Ricava le informazioni dell'host dalla richiesta ARP
                src_ip = stat.match['ipv4_src']
                src_mac = stat.match['eth_src']
                dpid = ev.msg.datapath.id
                port_no = stat.match['in_port']

                self.host_info[src_mac] = {
                    'ip': src_ip,
                    'dpid': dpid,
                    'port_no': port_no
                }
                self.logger.info(f"Host aggiunto: {src_mac} -> {src_ip}, Switch: {dpid}, Porta: {port_no}")

        self.get_topology()  # Ottieni la topologia aggiornata

    def _stats_csv(self):
        # Scrive le statistiche in un file CSV
        with open('port_stats.csv', 'a') as csvfile:
            fieldnames = ['time', 'dpid', 'port_no', 'rx_bytes', 'tx_bytes', 'rx_throughput', 'tx_throughput']
            writer = csv.DictWriter(csvfile, fieldnames=fieldnames)

            if csvfile.tell() == 0:  # Controlla se il file è vuoto
                writer.writeheader()
