...
class SimpleSwitch13(app_manager.RyuApp):
    ...
    def _block_port(self, dpid, port_no, src_mac):
      self.monitoring_list.remove((dpid, port_no))
      datapath = self.datapaths[dpid]
      parser = datapath.ofproto_parser
  
      # Crea una regola per bloccare solo i pacchetti provenienti dall'indirizzo MAC dell'attaccante
      match = parser.OFPMatch(in_port=port_no, eth_src=src_mac)  # Match sul MAC sorgente
      actions = []  # Nessuna azione, il traffico viene bloccato
  
      self.add_flow(datapath, 2, match, actions)  # Aggiungi la regola di blocco
      self.logger.info(f'Porta {(dpid, port_no)} bloccata per traffico sospetto da {src_mac}.')
    ...
    src_mac = self.mac_to_port[dpid].get(port_no)  # Ottieni l'indirizzo MAC della porta

    # Controlla se il traffico supera la soglia
    if rx_throughput > self.threshold:
        # Se la porta non è già in monitoring_list o in blocked_ports
        if (dpid, port_no) not in self.monitoring_list and (dpid, port_no) not in self.blocked_ports:
            ...
            self.logger.info(f'Porta {(dpid, port_no)} aggiunta a monitoring_list.')
            ...
            self._block_port(dpid, port_no, src_mac)  # Passa l'indirizzo MAC
